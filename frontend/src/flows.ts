...
export const FLOWS: Flow[] = [
  {
    ...
    label: "Generate",
    icon: Sparkles,
    headline: "From a repo URL to a running test suite",
    tagline:
      "Two AI agents, but only after a free pass that costs you nothing. You see what we found before a single test is written.",
    to: "/generate",
    cta: "Generate tests",
    steps: [
      {
        ...
        driver: "static",
        lane: "you",
        title: "You point us at your code",
        desc: "A GitHub URL — public or private — or a ZIP you drop in.",
        tags: ["POST /api/analyze", "POST /api/upload-zip"],
      },
      {
        id: "fetch",
        driver: "server",
        lane: "server",
        title: "We read every file",
        desc: "Nothing is cloned to disk — we pull the file tree and then the files straight into memory, 15 at a time.",
        tags: ["services/github_service.py", "concurrency=15"],
      },
      {
        id: "extract",
        driver: "server",
        lane: "server",
        title: "We throw away everything that isn't UI",
        desc: "No AI in this step — it runs on our server in milliseconds and costs nothing. We keep the files a browser actually renders, score each one 1–10 for how much it matters, and detect your stack.",
        tags: ["services/file_extractor.py"],
      },
      {
        id: "agent1",
        driver: "server",
        lane: "ai",
        title: "Agent 1 reads the project",
        desc: "The first model call. It answers in strict JSON: what this app is, its key pages, its routes, and the parts that will be awkward to test. If nothing renders in a browser we stop here and say so, rather than inventing a login page you don't have.",
        tags: ["agents/filter_agent.py", "JSON mode"],
      },
      {
        id: "preview",
        driver: "static",
        lane: "you",
        title: "You check we understood it",
        desc: "That analysis is what you see on screen — your chance to correct us before a credit is spent. The job is saved to SQLite, so it survives a restart.",
        tags: ["services/store.py", "job_id"],
      },
      {
        id: "agent2",
        driver: "client",
        lane: "ai",
        title: "Agent 2 writes the suite — grounded, not guessed",
        desc: "We pull the real selectors out of your source (ids, data-testid, names) and hand them to the model, then generate each file in its own call so a big suite can't truncate. It streams back token by token as it's written.",
        tags: ["GET /api/stream/{job_id}", "agents/writer_agent.py"],
      },
      {
        id: "scaffold",
        driver: "client",
        ...
        desc: "Config, dependency manifest, CI pipelines and a README are templated, not asked of an LLM — there's one correct answer per framework. Every selector the model wrote is checked back against your code, and anything invented is flagged.",
      ...
      },
      {
        ...
        desc: "Zipped in your browser — the files never make a second trip. Set BASE_URL to point the same suite at staging or prod; leave it unset and the config boots your app for you.",
